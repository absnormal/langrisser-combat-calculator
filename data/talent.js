var talent = [{
    ...
    */
    ...
    NAME: '孤芳永生',
    DESC:
    /* BUFF NOT SKILL */'行動結束時，如果自身2格範圍內沒有友軍部隊，則傷害提升20%，遭受傷害降低20%，持續1回合。若沒有處於[休眠]的狀態時，在遭受致命傷害時不會死亡，之後生命值恢復90%，並進入[休眠]：“無法行動，在效果結束時，恢復50%的生命”，持續1回合。（該效果無法免疫和驅散，每場戰鬥最多觸發1次）',
},{
    NAME: '勇者的意志',
    ODMGINC: 0.3,
    ...
    SKILL: function(side){
        ...
        if(oppPerHP >= 0.7) return [0.2, 0, 0, 0, 0];
        else return false;
    },
    ...
    SKILL: function(side){
        ...
        switch(run){
            case 0:
                return false;
            case 1:
                return [0.04, 0, 0.2, 0, 0];
            case 2:
                return [0.08, 0, 0.4, 0, 0];
            case 3:
                return [0.12, 0, 0.6, 0, 0];
            case 4:
                return [0.16, 0, 0.8, 0, 0];
            default:
                return [0.2, 0, 1, 0, 0];
        }
    },
    ...
    SKILL: function(side){
        ...
        if(perHP >= 0.5) return [0, 0, 0.2, 0, 0];
        else return false;
    },
    ...
    */
    ...
    SKILL: function(side){
        ...
        terrainLIST = ['樹林', '山地', '草地', '洞穴', '沼澤', '水中'];
        if(terrainLIST.includes(terrain.NAME)) return [0, 0.2, 0, 0.2, 0];
        else return false;
    },
    ...
    NAME: '情報分析',
    /* BUFF NOT SKILL */
    DESC: '行動結束時，令周圍4格內的3名敵軍部隊的防禦和魔防中數值較高的一項屬性降低10%，持續1回合。並令周圍3格內的4名其他友軍部隊的防禦和魔防中數值較低的一項屬性提升10%，持續1回合。',
},{
    NAME: '女將軍的鐵腕',
    /* BUFF NOT SKILL */
    DMGINC: 0.2,
    ...
    SKILL: function(side){
        ...
        armyLIST = ['飛兵', '騎兵'];
        if(armyLIST.includes(oppARMY)) return [0.3, 0, 0.3, 0, 0];
        else return false;
    },
